...
import eve.game.tracker.data.Repository
...
class RepositoryTest {
    ...
    @Test
    fun importingAnExportReplacesEverythingAndRestoresTheSession() = runBlocking {
        val anna = repo.addPlayer("Anna")
        val s = repo.startSession(gambioId(), listOf(anna))
        repo.recordScoreRound(s, mapOf(anna to 42))
        repo.endSession(s)
        ...
        repo.addPlayer("Temporary")
        repo.importJson(json)

        assertEquals(listOf("Anna"), repo.activePlayers().first().map { it.displayName })
        val restored = repo.liveSession(s).first()!!
        assertEquals(42, restored.standings.single().total)
        assertEquals(1, restored.roundCount)
    }

    @Test
    fun invalidImportLeavesTheExistingDatabaseUntouched() = runBlocking {
        repo.addPlayer("Anna")
        val invalid = """
            {
              "format": 2,
              ...
              "players": [],
              "sessions": [{
                "id": 1, "gameId": 999, "startedAt": 1,
                "players": [], "rounds": [], "ledger": []
              }]
            }
        """.trimIndent()

        try {
            repo.importJson(invalid)
            throw AssertionError("Invalid import should fail")
        } catch (_: IllegalArgumentException) {
            // expected
        }

        assertEquals(listOf("Anna"), repo.activePlayers().first().map { it.displayName })
        assertEquals(5, repo.enabledGames().first().size)
    }
}
